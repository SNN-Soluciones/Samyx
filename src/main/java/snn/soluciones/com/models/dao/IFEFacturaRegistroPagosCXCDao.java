...
public interface IFEFacturaRegistroPagosCXCDao extends CrudRepository<FEFacturaRegistroPagosCXC, Long> {
  ...
  @Query("""
  SELECT c FROM FEFacturaRegistroPagosCXC c 
  WHERE c.emisor.id = ?1 
  AND c.id = (
    SELECT MAX(c2.id) 
    FROM FEFacturaRegistroPagosCXC c2 
    WHERE c2.emisor.id = ?1
  )
""")
  FEFacturaRegistroPagosCXC findMaxFacturaByEmisor(Long emisorId);
  ...
  List<FEFacturaRegistroPagosCXC> reporteFlujoCajaCXC(@Param("emisorId") Long paramLong, @Param("f1") Date paramDate1, @Param("f2") Date paramDate2, @Param("mp") List<Long> paramList);
}
