...
public class LoginController {
  ...
  public String login(@RequestParam(value = "error", required = false) String error, @RequestParam(value = "logout", required = false) String logout, Model model, Principal principal, RedirectAttributes flash) {
    if (principal != null) {
      flash.addFlashAttribute("info", "Ya ha iniciado sesianteriormente");
      return "redirect:/";
    } 
    model.addAttribute("titulo", "Iniciar sesión con existo");
    if (error != null)
      model.addAttribute("error", "Error en el login: Nombre de usuario o contraseña incorrecta, por favor vuelva a intentarlo!");
    if (logout != null)
      model.addAttribute("info", "Has cerrado con la sesión correctamente!");
    return "/login/login";
  }
}
