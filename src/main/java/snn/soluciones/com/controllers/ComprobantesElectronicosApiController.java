...
public class ComprobantesElectronicosApiController {
  ...
  public String comprobantesUsuario(Model model, @RequestParam(name = "page", defaultValue = "0") int page, @RequestParam(name = "q", defaultValue = "") String q, HttpSession session) {
    if (session.getAttribute("SESSION_EMPRESA_ID") != null) {
      ...
      PageRequest pageRequest = PageRequest.of(page, 40);
      Page<ComprobantesElectronicos> listaComprobantesElectronicos = this._comprobantesElectronicosService.findAllComprobanteByEmisor(emisor.getIdentificacion(), q.toUpperCase(), (Pageable)pageRequest);
      PageRender<ComprobantesElectronicos> pageRender = new PageRender("/comprobantes-electronicos-enviados", listaComprobantesElectronicos);
      model.addAttribute("listaComprobantesElectronicos", listaComprobantesElectronicos);
      model.addAttribute("page", pageRender);
      model.addAttribute("urlApiDownloadDocs", this.urlApiDownloadDocs);
      model.addAttribute("urlApiImprimirPdf", this.urlApiImprimirPdf);
  ...
  public String save(Model model, ComprobantesElectronicos ce, BindingResult result, SessionStatus status) {
    this._comprobantesElectronicosService.UpdateComprobantesElectronicosApi(ce.getResponseCodeSend(), ce.getIndEstado(), ce.getEmailDistribucion(), ce.getReconsultas(), ce.getId());
    status.setComplete();
